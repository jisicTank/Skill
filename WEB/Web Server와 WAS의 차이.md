...
HTTP를 통해 애플리케이션을 수행해주는 미들웨어다. (Client - **MiddleWare Server** - DB Server(DBMS))
...
1. 웹 서버에서는 정적 컨텐츠만 처리하도록 기능 분배를 해서 서버 부담을 줄이는 것

```
클라이언트에 이미지 파일(정적 컨텐츠)를 보낼 때..
...
![sss](https://user-images.githubusercontent.com/24764210/110680456-fb7e0500-821b-11eb-9d28-20d9c289a4f4.jpg)
...
동적 컨텐츠를 처리 할 수 있는 가이다.

웹 서버의 경우는 정적 컨텐츠를 처리하지만, WAS의 경우는 정적/동적 컨텐츠를 처리할 수 있다.

그러나 WAS가 정적 컨텐츠까지 처리하다보면 서버에 부하가 많이 걸리므로 DB와 비즈니스 로직과 같은 동적 컨텐츠만 처리하는 것이 효율적이다. 

<br>

### 웹서버와 WAS를 구분하는 이유는?

- 기능을 분리함으로써 서버의 부하를 방지한다.
- 물리적으로 분리하여 WAS의 환경설정 파일을 외부에 노출시키지 않을 수 있어 보안이 강화된다.
- 여러 대의 WAS를 연결할 수 있어 오류가 발생했을 때 장애를 극복할 수 있다.
